...
class WebVitalsInstrumentation extends EmbraceInstrumentationBase.EmbraceInstrumentationBase {
    // instrumentation that adds an event to the session span for each web vital report
    constructor({ diag, perf, trackingLevel = 'core', listeners = constants.WEB_VITALS_ID_TO_LISTENER, urlDocument = window.document, urlAttribution = false } = {}){
        super({
        ...
        this._metricsToTrack = trackingLevel === 'core' ? [
            ...constants.CORE_WEB_VITALS
        ] : [
            ...constants.ALL_WEB_VITALS
        ];
    ...
    enable() {
        this._metricsToTrack.forEach((name)=>{
            this._listeners[name]?.((metric)=>{
                ...
                const attrs = {
                    [attributes.KEY_EMB_TYPE]: attributes.EMB_TYPES.WebVital,
                    [semanticConventions.ATTR_URL_FULL]: this._getAttributedURLForMetric(metric),
                ...
                currentSessionSpan.addEvent(`${constants.EMB_WEB_VITALS_PREFIX}-report-${name}`, attrs, metricTime);
            });
        });
