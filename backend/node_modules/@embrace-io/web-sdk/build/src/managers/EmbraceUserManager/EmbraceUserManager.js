...
var index = require('../../api-users/manager/constants/index.js');
...
class EmbraceUserManager {
    ...
    setEmbraceUserId(userId) {
        ...
        try {
            this._storage.setItem(constants.EMBRACE_USER_ID_STORAGE_KEY, userId);
        } catch (e) {
    ...
    clearEmbraceUserId() {
        this._embraceUserId = null;
        try {
            this._storage.removeItem(constants.EMBRACE_USER_ID_STORAGE_KEY);
        } catch (e) {
    ...
    _migrateOldLocalStorageKey() {
        ...
        try {
            const oldUserStorage = this._storage.getItem(constants.EMBRACE_USER_STORAGE_KEY_DEPRECATED);
            if (oldUserStorage) {
                const user = JSON.parse(oldUserStorage);
                if (types.isUser(user)) {
                    this._diag.debug('Migrating old user data from storage');
                    this._storage.setItem(constants.EMBRACE_USER_ID_STORAGE_KEY, user[index.KEY_ENDUSER_PSEUDO_ID]);
                    this._storage.removeItem(constants.EMBRACE_USER_STORAGE_KEY_DEPRECATED);
                } else {
    ...
    _initialSetup() {
        try {
            const embraceUserId = this._storage.getItem(constants.EMBRACE_USER_ID_STORAGE_KEY);
            if (!embraceUserId) {
                this._diag.debug('No existing user found in storage, creating a new one');
            } else if (types.isUserId(embraceUserId)) {
    ...
    _generateNewEmbraceUserId() {
        const newUserId = generateUUID.generateUUID();
        this.setEmbraceUserId(newUserId);
        return newUserId;
    }
    // This is the external user id that can be set by the user
    getUserId() {
        return this._storage.getItem(constants.EMBRACE_EXTERNAL_USER_ID_KEY);
    }
    // Use storage as source of truth so multiple tabs can share the same user id
    setUserId(userId) {
        this._storage.setItem(constants.EMBRACE_EXTERNAL_USER_ID_KEY, userId);
    }
    clearUserId() {
        this._storage.removeItem(constants.EMBRACE_EXTERNAL_USER_ID_KEY);
    }
}

exports.EmbraceUserManager = EmbraceUserManager;
